    ...
    if request.method == "POST":
        ...
        if incoming_msg is not None:
            if incoming_msg.startswith("/image"):
                response = client.send_normal_message.delay(self=client, replay="Image will be added soon",
                                                            phone_number=user)
            else:
                ...
                session['chat_log'] = append_interaction_to_chat_log(incoming_msg, answer,
                                                                     chat_log)
                if user is not None:
                    response = client.send_normal_message.delay(self=client, replay=answer, phone_number=user)
                else:
                    response = "No User"
        else:
            response = "No Data"
